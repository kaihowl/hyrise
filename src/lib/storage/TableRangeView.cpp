...
size_t TableRangeView::getStart() const { return _start; }
...
size_t TableRangeView::partitionWidth(const size_t slice) const { return _table->partitionWidth(slice); }
...
table_id_t TableRangeView::subtableCount() const { return 1; }

atable_ptr_t TableRangeView::copy() const { return std::make_shared<TableRangeView>(_table, _start, _end); }

atable_ptr_t TableRangeView::copy_structure(const field_list_t* fields,
...
DataType TableRangeView::typeOfColumn(const size_t column) const { return _table->typeOfColumn(column); }

size_t TableRangeView::columnCount() const { return _columnCount; }
